...
source("../rscripts/proctiers.r")

##-----------------------------------------------------------------
## wrappers
get.f0.tiers.conv <- function(currconv, pdir) {
	y <- currconv
	xtier.list <- get.tiers(pdir,suffix=".PitchTier", yname="F0", short=F, skip=3)  
	return(xtier.list)
}
...
normalize.conv <- function(x, x.aggs, var.name="F0", st=c("mean.val", "q5.val", "none"), zscore=F, center=c("mean","min","none"), 
	...
	print("normalize.conv")
		...
		print(currpart)
		curr.norm <- normalize.vals.spk(u[participant==currpart], x.aggs, var.name=var.name, st=st, zscore=zscore, center=center, remove.outliers=remove.outliers, remove.spurt.slope=remove.spurt.slope)	
...
normalize.vals.spk <- function(u, x.aggs, var.name="F0", st=c("mean.val", "q5.val", "none"), zscore=F, center=c("mean","min","none"), 
	...
	if (remove.spurt.slope==T) {
		...
		print(c(nrow(xslope), nrow(norm.u)))
...
#add.dI0.conv <- function(x) {
#        return(x[,{   
#		if (length(normI0) >= 4) {
#			dy <- get.spline.deriv(Time, normI0)
#			dys <- get.spline.deriv(Time, slopeI0)
#			xcurr <- data.table(
#				nxt_agent=nxt_agent, conv=conv, participant=participant, channel=channel, conv_id=conv,
#				starttime=starttime, endtime=endtime,
#				Time=Time, I0=I0, normI0=normI0, normI0.slope=slopeI0, maxtime=maxtime,
#				dI0=dy$y, dI0.slope=dys$y)
#		} else {
#			xcurr <- data.table(
#				nxt_agent=nxt_agent, conv=conv, participant=participant, channel=channel, conv_id=conv,
#				starttime=starttime, endtime=endtime,
...
#
#
#add.dF0.conv <- function(x) {
#        u <- x[,{    
#		if (length(normF0) >= 4) {
#			dy <- get.spline.deriv(Time, normF0)
#			dys <- get.spline.deriv(Time, slopeF0)
#
#			xcurr <- data.table(
#				nxt_agent=nxt_agent, conv=conv, participant=participant, channel=channel, conv_id=conv,
#				starttime=starttime, endtime=endtime,
#				Time=Time, F0=F0, normF0=normF0, normF0.slope=slopeF0, maxtime=maxtime,
#				dF0=dy$y, dF0.slope=dys$y)
#		} else {
#			xcurr <- data.table(
#				nxt_agent=nxt_agent, conv=conv, participant=participant, channel=channel, conv_id=conv,
#				starttime=starttime, endtime=endtime,
...
#get.f0.tiers <- function(segdir) {
#        convs <- data.frame(conv=list.files(segdir))
#        ptiers <- dlply(convs, .(conv), get.f0.tiers.conv, xdir=segdir)
#
#        return(ptiers)
#}

#get.intensity.tiers <- function(segdir) {
#
#        convs <- data.frame(conv=list.files(segdir))
#        ptiers <- dlply(convs, .(conv), function(x) {
#                        y <- unlevel(x$conv)
#                        print(y)
#                        pdir <- paste(segdir,"/",y,"/",y,"-int", sep="")
#                } )
#
#        return(ptiers)
#}
